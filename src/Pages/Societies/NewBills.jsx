    ...
    const prevCharges = fetchedBillData.map((item)=>{
      let latestObj =  latestCharge.find(ele=>ele.memberId == item.memberId);
       let differenceInTimes =
                ...
                (latestObj.charges.length > 0 ? new Date(latestObj.charges[0].date).getTime():null);

            let  dayDiff = (latestObj.charges.length > 0 ? Math.ceil(
...
const createChargeObject = (value)=>{
          ...
          let filteredLatestBill = billData.map((bill) => {
            ...
            return {
              ...
              heads:
                item && item.charges.length > 0 ? item.charges[0].heads : heads,
              totalWithoutInt:
          ...
          let mergedBillData = filteredLatestBill.map((item) => {
            ...
            let openBal = openingBal.filter(
              (ele) => ele.memberId === item.memberId
            );
    ...
    const fetchMember = async () => {
      try {
        ...
        let arr = res.data.map((item, index) => {
          ...
          let filteredHeads = mergedBillData.filter(ele=>ele.memberId == item._id);
  ...
  const handleHeadValueChange = (memberIndex, billHead, value) => {
    ...
    setBillData((prevBillData) => {
      ...
      charges[0] = {
        ...charges[0],
        heads,
        total: Number(Object.values(heads)
          .reduce((sum, headValue) => sum + Number(headValue), 0))+Number(charges[0].prevDue)+Number(charges[0].interest),
        intAppliedHeadSum: sumOfInterestHeads,
        headTotal: Number(Object.values(heads)
          .reduce((sum, headValue) => sum + Number(headValue), 0)),
      };
    ...
    return billData.map((member, index) => {
     ...
     const charge =
       mergedBillData && mergedBillData.length > 0
         ? mergedBillData.find((ele) =>{ if(!ele.memberId){ return null} 
        return ele.memberId === member.memberId})
         ...
         console.log(charge);
          ...
          member.charges[0].prevDue = charge
          ...
          member.charges[0].intAppliedHeadSum = charge
            ? sumOfInterestHeads
            : 0;
...
const handleSendEmail = async () => {
  ...
  for (let item of objForEmail) {
    if (selectedItems.includes(item.memberId) && !sentEmails.has(item.email)) {
      try {
